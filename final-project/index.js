...
nameOne.addEventListener("keyup", event => {
  ...
  initialOne.innerHTML = nameOne.value.slice(0,1).toUpperCase()
})
nameTwo.addEventListener("keyup", (event) => {
  nameTwoInvite.innerHTML = nameTwo.value
  initialTwo.innerHTML = nameTwo.value.slice(0,1).toUpperCase()
})
...
function formal() {
  document.getElementById("invitation").className = "formal"
}
function spring() {
  document.getElementById("invitation").className = "spring"
}
function artDeco() {
 document.getElementById("invitation").className = "artDeco" 
}
